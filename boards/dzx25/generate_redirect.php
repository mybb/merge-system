...
class DZX25_Redirect_Generator
{
	...
	{
		if(!isset($this->redirect_file_name[$module]) || empty($this->redirect_file_name[$module]))
		{
			echo "An error prevents creating the redirect file: no such module '{$module}'.<br />";
			return;
		}
		$file_path = dirname(__FILE__) . "/" . $this->redirect_file_name[$module];
...
{
	$ret = array(
			'index' => 'DZX25_dzx_redirect_index.php',
			'home' => 'DZX25_dzx_redirect_home.php',
	);
	return $ret;
}
...
if(isset($dz_id))
{
	...
	DZX_Redirect_User::redirect($redirect_url);
}
